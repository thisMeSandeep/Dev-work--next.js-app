...
const ProposalsTable = ({ jobId }: { jobId: string }) => {
    ...
    const [selectedProposal, setSelectedProposal] = useState<ProposalWithUser | null>(null);
    const [viewType, setViewType] = useState<"proposal" | "developer" | null>(null);
    ...
    const developerData = {
        ...
        experienceLevel: selectedProposal?.freelancerProfile.experienceLevel ?? null,
    ...
    return (
        <div className="p-5">
            <h1 className="text-2xl font-semibold mb-5">Proposals received for this job</h1>

            <Table>
                <TableCaption>{proposals.length} Proposals received</TableCaption>
                <TableHeader>
                    <TableRow>
                ...
                <TableBody>
                    {proposals.map((proposal) => (
                        <TableRow key={proposal.id}>
                            <TableCell>{proposal.freelancerProfile.user.firstName + " " + proposal.freelancerProfile.user.lastName}</TableCell>
            ...
            {openPopup && (
                <Popup
                    headerTitle={viewType === "developer" ? "Developer Profile" : "Proposal Details"}
                    onClose={() => {
                        setOpenPopup(false);
                        setViewType(null);

...
const DeveloperProfile = () => {
  ...
  const textPlaceholder = (user?.firstName?.[0] ?? "") + (user?.lastName?.[0] ?? "");

  // developer stats
  const stats = {
    name: `${user?.firstName ?? ""} ${user?.lastName ?? ""}`.trim(),
    email: user?.email ?? "",
    jobsApplied: user?.FreelancerProfile?.jobsApplied ?? 0,
    jobsHired: user?.FreelancerProfile?.totalJobsHired ?? 0,
    totalEarnings: user?.FreelancerProfile?.totalIncome ?? 0,
  };
